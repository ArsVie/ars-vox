...
| `youtube` | "the LLM searches YouTube and OFFERS the user options (results render as selectable cards). The user picks one (click or voice)" | PENDING | W1-YOUTUBE | `probes/youtube_realness.py` (FAIL today: FIXTURE_RESULTS still present) | `test_agent_search_emits_youtube_search_offer` | OFFER channel verified (agent tool → `youtube.search` with selectable cards); REAL provider seam, zero results answered honestly ("no encontré nada", never a fixture); packaged: ask for a video, pick one by voice, it plays in the media panel |
