...
$log = Join-Path $root "results\logs\cuda-win-set.log"
& $py tools\stt_set.py data\baseline --models $small $turbo --device cuda --compute-type float16 --out results\archive\cuda-win 2>&1 | Out-File -Encoding utf8 $log
