    ...
    const applyEvent = (event: ServerEvent): void => {
      ...
      switch (event.type) {
        ...
        case "state_snapshot": {
          ...
          set(patch);
          // R19/R33 (GATE-3.5): the reconnect source enters the ONE choke
          // when the snapshot carries the adaptive composition. This is the
          // frontend half of the seam — A6 lands the Python wire half with
          // the same field name (adaptive_composition: { spec, overrides }).
          // userInitiated: the snapshot is authoritative server truth; a
          // reload/reconnect reconstructs the workspace (inertia must not
          // damp an authoritative restore).
          const adaptiveSnap = (snap as StateSnapshotEvent).adaptive_composition;
          if (adaptiveSnap?.spec) {
            // Defensive: a malformed/foreign snapshot must never crash the
            // reconnect path — the choke's contract (invalid specs never
            // reach state) is honored by SKIPPING the restore instead.
            try {
              applyAdaptiveSpec(adaptiveSnap.spec, {
                overrides: adaptiveSnap.overrides ?? EMPTY_OVERRIDES,
                userInitiated: true,
              });
            } catch {
              // snapshot composition ignored — existing reconnect behavior
            }
          }
          recompute();
          return;
